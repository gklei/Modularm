...
static CGPoint _midPoint(CGPoint point1, CGPoint point2)
{
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.enableGraduations == YES) {
        for (int i = 0; i<60; i++) {
           ...
           if (self.graduationLength == -1)
           {
              UIBezierPath* circlePath = [UIBezierPath bezierPathWithArcCenter:_midPoint(P1, P2) radius:2 startAngle:0 endAngle:2*M_PI clockwise:YES];
              [circlePath fillWithBlendMode:kCGBlendModeNormal alpha:self.graduationAlpha];
           }
           else
           {
